...
export class TransformService {
  async transform(body: { source?: string; outputs?: string[]; audience?: string; tone?: string; language?: string; detail?: string; research?: boolean }) {
...
Return a concise JSON object with keys: title, summary, facts (array), entities (array), outputs (object keyed by requested output names), quality (object with score 0-100, passed boolean, issues array). For Presentation, include slides as an array with title, bullets and speakerNotes. For Video Package include script, storyboard and narration. Keep every output grounded in the same facts. Never omit a requested output.`;
    ...
    try {
      ...
      // Deterministic Quality Guardian checks supplement the model's self-assessment.
      ...
      const score = Number.isFinite(modelScore) ? Math.max(0, Math.min(100, Math.round(modelScore))) : 70;
      content.quality = { score: passed ? score : Math.min(score, 69), passed, issues };
      return { ok: true, status: passed ? "verified" : "review_required", content };
    } catch (error) {
