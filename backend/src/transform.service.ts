...
export class TransformService {
  ...
  async transform(body: { source?: string; outputs?: string[]; audience?: string; tone?: string; language?: string; detail?: string; research?: boolean; model?: string; verify?: boolean; researchEvidence?: string; researchSources?: Array<{title:string;url:string;snippet:string;score?:number}> }) {
    ...
    try {
      ...
      const agentResult = await this.agentOrchestrator.run({ source: workingSource, title:base.title, summary:base.summary, facts:base.facts, entities:base.entities, claims:base.claims, provenance:base.provenance, audience:body.audience, tone:body.tone, language:body.language, detail:body.detail }, outputs, model);
      ...
      if(body.verify!==false){
        try{
          const p=`You are the Quality Guardian. Verify generated content against ORIGINAL SOURCE and RESEARCH EVIDENCE. For each material claim, check whether it is supported by a source. Return ONLY JSON {"score":0-100,"passed":true|false,"issues":["..."],"claim_checks":[{"claim":"...","supported":true,"source_ids":["source-1"],"reason":"..."}]}. Flag unsupported claims, contradictions, missing requested outputs, or obvious format failures.\nORIGINAL SOURCE:\n${source.slice(0,45000)}\nRESEARCH:\n${evidence.slice(0,12000)}\nREQUESTED:${outputs.join(", ")}\nGENERATED:\n${JSON.stringify(content).slice(0,50000)}`;
  ...
  async refine(body:{source?:string;outputs?:string[];content?:unknown;issues?:string[];audience?:string;tone?:string;language?:string;detail?:string;model?:string}) {
    ...
    try{
      ...
      const guardianPrompt=`You are the final Quality Guardian. Compare refined content ONLY against the ORIGINAL SOURCE. Return ONLY JSON {"score":0-100,"passed":true|false,"issues":["..."],"claim_checks":[{"claim":"...","supported":true,"source_ids":["source-1"],"reason":"..."}]}. Flag unsupported claims, contradictions, missing requested outputs, or obvious format failures.\nORIGINAL SOURCE:\n${source.slice(0,45000)}\nREQUESTED:${outputs.join(", ")}\nREFINED:\n${JSON.stringify(refined).slice(0,55000)}`;
  ...
  private async buildIntelligence(google:any, model:string, source:string, outputs:string[], body:any){
    const prompt=`You are the Content Intelligence Agent. Analyze the source and return ONLY JSON {"title":"...","summary":"...","facts":[{"id":"f1","text":"...","source_ids":["source-1"]}],"entities":[{"name":"...","description":"..."}],"claims":[{"id":"c1","text":"...","source_ids":["source-1"],"confidence":0.0}],"provenance":{"f1":[{"source_id":"source-1","source_name":"...","excerpt":"..."}]},"outputs":{}}. Extract source IDs/names from SOURCE_ID and SOURCE_NAME markers. Every fact and claim should cite one or more source_ids and a short supporting excerpt in provenance when available. Keep outputs empty here; specialized agents will generate them. Never invent facts.\nSOURCE:\n${source}\nREQUESTED:${outputs.join(", ")}\nAUDIENCE:${body.audience||"General audience"}\nLANGUAGE:${body.language||"English"}`;
    const {text}=await generateText({model:google(model),prompt,temperature:.1});
    try{return JSON.parse(text.replace(/^```json\s*/i,"").replace(/```$/i,"").trim());}catch{return {title:"Transformation",summary:text,facts:[],entities:[],claims:[],provenance:{},outputs:{}};}
  }
