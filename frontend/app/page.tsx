...
type OutputValue = string | { title?: string; sections?: { heading?: string; content?: string }[]; key_points?: string[]; visual_elements_suggestion?: string[]; slides?: { title?: string; bullets?: string[]; speakerNotes?: string }[]; script?: string; storyboard?: { scene?: string; visual?: string; narration?: string }[]; narration?: string };
...
function OutputCard({ name, value, onExport }: { name: string; value: OutputValue; onExport: (kind: string, value: OutputValue) => void }) {
  ...
  return <div className="artifact-card">
    ...
    {typeof value === "string" ? <p className="artifact-text">{value}</p> : <>
      ...
      {value.key_points?.length ? <div className="suggestions"><strong>Key points</strong>{value.key_points.map(x => <span key={x}>✓ {x}</span>)}</div> : null}
      {value.slides?.map((slide, index) => <div className="slide-card" key={`${slide.title}-${index}`}><span>Slide {index + 1}</span><strong>{slide.title}</strong>{slide.bullets?.map(b => <p key={b}>• {b}</p>)}{slide.speakerNotes && <small>Speaker notes: {slide.speakerNotes}</small>}</div>)}
...
export default function Home() {
  const [source, setSource] = useState(""); const [url, setUrl] = useState("");
  const [selected, setSelected] = useState(["LinkedIn Post", "Presentation"]); const [content, setContent] = useState<Content | null>(null);
  const [status, setStatus] = useState("Ready"); const [research, setResearch] = useState(false); const [fileName, setFileName] = useState(""); const [dragging, setDragging] = useState(false);
  ...
  async function useUrl() {
    if (!url.trim()) return; setStatus("Fetching URL…");
    try { const r = await fetch(`${API}/api/source/ingest`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url }) }); const d = await r.json(); if (!r.ok || !d.content) throw Error(d.message || "Could not read URL"); setSource(d.content); setFileName(`URL · ${url}`); setStatus("URL loaded"); }
    catch (e) { setStatus(e instanceof Error ? e.message : "URL fetch failed"); }
  }

  async function upload(file?: File) {
    if (!file) return; setFileName(file.name); setStatus(`Extracting ${file.name}…`);
    const form = new FormData(); form.append("file", file);
    try { const r = await fetch(`${API}/api/sources/upload`, { method: "POST", body: form }); const d = await r.json(); if (!r.ok || d.ok === false) throw Error(d.message || "Upload failed"); if (d.content) setSource(d.content); setStatus(d.type === "image" || d.type === "video" ? "Media accepted" : "Source extracted"); }
    catch (e) { setStatus(e instanceof Error ? e.message : "Upload failed"); }
  }

  async function generate() {
    if (!source.trim() || !selected.length) return; setStatus("Generating → verifying…"); setContent(null);
    try {
      const r = await fetch(`${API}/api/transform`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source, outputs: selected, research, verify: true, ...controls }) });
      const d = await r.json().catch(() => ({ message: "Backend returned an invalid response." })); if (!r.ok || d.ok === false) throw Error(d.message || `Request failed with HTTP ${r.status}`);
      setContent(d.content); setStatus(d.status === "verified" ? "Verified" : "Review required");
      try { const old = JSON.parse(localStorage.getItem("transformai-history") || "[]"); const item = { id: crypto.randomUUID(), createdAt: new Date().toISOString(), sourcePreview: source.slice(0, 180), outputs: selected, content: d.content }; localStorage.setItem("transformai-history", JSON.stringify([item, ...old].slice(0, 20))); } catch {}
