...
export default function Home() {
  ...
  async function download(kind: string, value: OutputValue) {
    ...
    try {
      const endpoint = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/export/${kind}`;
      let payload: any = { title: content?.title || "TransformAI Output" };
      ...
      const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = kind === "pptx" ? "transformai-presentation.pptx" : kind === "pdf" ? "transformai-report.pdf" : "transformai-report.docx"; a.click(); URL.revokeObjectURL(url);
