...
export type WorkflowSnapshot = { nodes: Node[]; edges: Edge[]; config: Record<string, NodeConfig> };
...
export default function WorkflowCanvas({ onConfigChange, snapshot, onSnapshotChange }: { onConfigChange?: (config: Record<string, NodeConfig>) => void; snapshot?: WorkflowSnapshot | null; onSnapshotChange?: (snapshot: WorkflowSnapshot) => void }) {
  const [nodes, setNodes, onNodesChange] = useNodesState(snapshot?.nodes || initialNodes); const [edges, setEdges, onEdgesChange] = useEdgesState(snapshot?.edges || initialEdges); const [selected, setSelected] = useState<string | null>(null);
  const selectedNode = useMemo(() => nodes.find(n => n.id === selected), [nodes, selected]);
  const emit = useCallback((nextNodes: Node[], nextEdges: Edge[]) => {
    const config = Object.fromEntries(nextNodes.map(n => [n.id, (n.data?.config || {}) as NodeConfig]));
    onConfigChange?.(config); onSnapshotChange?.({ nodes: nextNodes, edges: nextEdges, config });
  }, [onConfigChange, onSnapshotChange]);
  useEffect(() => { if (!snapshot) return; setNodes(snapshot.nodes || initialNodes); setEdges(snapshot.edges || initialEdges); onConfigChange?.(snapshot.config || Object.fromEntries((snapshot.nodes || initialNodes).map(n => [n.id, n.data?.config || {}]))); }, [snapshot, setNodes, setEdges, onConfigChange]);
  const handleNodesChange = useCallback((changes:any) => { onNodesChange(changes); setTimeout(() => setNodes(current => { const next=[...current]; emit(next, edges); return next; }), 0); }, [onNodesChange, setNodes, emit, edges]);
  const handleEdgesChange = useCallback((changes:any) => { onEdgesChange(changes); setTimeout(() => setEdges(current => { const next=[...current]; emit(nodes, next); return next; }), 0); }, [onEdgesChange, setEdges, emit, nodes]);
  const onConnect = useCallback((connection: Connection) => setEdges(eds => { const next=addEdge({ ...connection, animated:true }, eds); emit(nodes, next); return next; }), [setEdges, emit, nodes]);
  function updateConfig(patch: NodeConfig) {
    if (!selected) return;
    setNodes(ns => { const next=ns.map(n => n.id===selected ? { ...n, data:{ ...n.data, config:{ ...(n.data.config as NodeConfig), ...patch } } } : n); emit(next, edges); return next; });
  }
  return <div className="relative h-[620px] overflow-hidden rounded-2xl border border-white/10 bg-[#0b1020]">
    <ReactFlow nodes={nodes} edges={edges} onNodesChange={handleNodesChange} onEdgesChange={handleEdgesChange} onConnect={onConnect} onNodeClick={(_, n) => setSelected(n.id)} fitView minZoom={0.4} maxZoom={1.5} defaultEdgeOptions={{ style: { strokeWidth: 2 } }}>
    ...
    {selectedNode && <div className="absolute right-4 top-4 z-10 w-80 rounded-xl border border-white/10 bg-[#11182b]/95 p-4 shadow-2xl backdrop-blur">
      ...
      {selected === "outputs" && <div className="mt-3 space-y-2"><select className="w-full rounded-lg bg-[#09090b] p-2 text-xs text-white" value={(selectedNode.data.config as NodeConfig)?.audience || "General audience"} onChange={e => updateConfig({ audience:e.target.value })}><option>General audience</option><option>Executive / leadership</option><option>Technical</option><option>Public / social</option></select><select className="w-full rounded-lg bg-[#09090b] p-2 text-xs text-white" value={(selectedNode.data.config as NodeConfig)?.tone || "Professional"} onChange={e => updateConfig({ tone:e.target.value })}><option>Professional</option><option>Concise</option><option>Formal</option><option>Engaging</option></select><select className="w-full rounded-lg bg-[#09090b] p-2 text-xs text-white" value={(selectedNode.data.config as NodeConfig)?.detail || "Balanced"} onChange={e => updateConfig({ detail:e.target.value })}><option>Brief</option><option>Balanced</option><option>Detailed</option></select></div>}
      <p className="mt-3 text-xs leading-5 text-slate-500">Drag nodes, connect handles, and configure execution behavior. The graph can be saved and restored locally.</p><button onClick={() => setSelected(null)} className="mt-3 rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300">Close</button>
    </div>}
  </div>;
}
